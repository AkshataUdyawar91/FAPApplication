...
class _InvoiceListSectionState extends State<InvoiceListSection> {
  ...
  Future<void> _pickCostSummaryFile(int invoiceIndex, int campaignIndex) async {
    try {
      ...
      if (result != null && result.files.isNotEmpty) {
        setState(() => _invoices[invoiceIndex].campaigns[campaignIndex].costSummaryFile = result.files.first);
        widget.onInvoicesChanged(_invoices);
      }
    } catch (e) {
  ...
  Future<void> _pickPhotos(int invoiceIndex, int campaignIndex) async {
    try {
      ...
      if (result != null && result.files.isNotEmpty) {
        setState(() => _invoices[invoiceIndex].campaigns[campaignIndex].photos.addAll(result.files));
        widget.onInvoicesChanged(_invoices);
      }
    } catch (e) {
  ...
  void _removePhoto(int invoiceIndex, int campaignIndex, int photoIndex) {
    setState(() => _invoices[invoiceIndex].campaigns[campaignIndex].photos.removeAt(photoIndex));
    widget.onInvoicesChanged(_invoices);
  }
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
          ...
          children: [
            Row(
              children: [
                ...
                Text('Invoices (${_invoices.length})', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
              ],
            ),
            ElevatedButton.icon(
        ...
        // Invoice list
        ...List.generate(_invoices.length, (i) => _buildInvoiceCard(_invoices[i], i, _expandedInvoiceIndex == i)),
      ],
    );
  }
    ...
    return Container(
      ...
      child: Column(
        children: [
          // Header
          InkWell(
            onTap: () => setState(() => _expandedInvoiceIndex = isExpanded ? -1 : index),
              ...
              child: Row(
                children: [
                  Container(
                    width: 32, height: 32,
                    decoration: BoxDecoration(
                      color: invoice.file != null ? AppColors.approvedBackground : AppColors.primary.withOpacity(0.1),
                      shape: BoxShape.circle,
                    ),
                    child: Center(
                      child: invoice.file != null
                          ? const Icon(Icons.check, color: AppColors.approvedText, size: 18)
  ...
  Widget _buildInvoiceFileUpload(InvoiceData invoice, int index) {
      ...
      return InkWell(
        ...
        child: Container(
          ...
          child: Row(
            ...
            children: [
              Icon(Icons.cloud_upload_outlined, color: AppColors.primary.withOpacity(0.6)),
              const SizedBox(width: 8),
              const Text('Click to upload Invoice (PDF)', style: TextStyle(color: AppColors.primary, fontWeight: FontWeight.w500)),
            ],
          ),
        ),
      );
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: AppColors.approvedBackground, borderRadius: BorderRadius.circular(8), border: Border.all(color: AppColors.approvedBorder)),
      child: Row(
        children: [
          const Icon(Icons.check_circle, color: AppColors.approvedText, size: 20),
          const SizedBox(width: 8),
          Expanded(child: Text(invoice.file!.name, style: const TextStyle(fontWeight: FontWeight.w500), overflow: TextOverflow.ellipsis)),
          IconButton(
            onPressed: () { setState(() => invoice.file = null); widget.onInvoicesChanged(_invoices); },
            icon: const Icon(Icons.close, size: 18), padding: EdgeInsets.zero, constraints: const BoxConstraints(),
          ),
        ],
      ),
    );
  ...
  Widget _buildInvoiceFields(InvoiceData invoice) {
    return Wrap(
      ...
      children: [
        _buildField('Invoice No.', invoice.invoiceNumber, (v) { invoice.invoiceNumber = v; widget.onInvoicesChanged(_invoices); }),
        _buildField('Invoice Date', invoice.invoiceDate, (v) { invoice.invoiceDate = v; widget.onInvoicesChanged(_invoices); }, hint: 'dd-mm-yyyy'),
        _buildField('Amount (₹)', invoice.totalAmount, (v) { invoice.totalAmount = v; widget.onInvoicesChanged(_invoices); }),
        _buildField('GST Number', invoice.gstNumber, (v) { invoice.gstNumber = v; widget.onInvoicesChanged(_invoices); }),
      ],
    );
  }

  Widget _buildField(String label, String value, Function(String) onChanged, {String? hint, double width = 180, bool readOnly = false}) {
    return SizedBox(
      width: width,
      child: TextField(
        ...
        decoration: InputDecoration(
          ...
          isDense: true,
          filled: readOnly, fillColor: readOnly ? Colors.grey.shade100 : null,
        ),
    ...
    return Column(
      ...
      children: [
        Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            const Text('Campaigns', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: AppColors.primary)),
            TextButton.icon(
        ...
        ...List.generate(invoice.campaigns.length, (i) => _buildCampaignCard(invoice.campaigns[i], invoiceIndex, i)),
      ],
    );
  }

  Widget _buildCampaignCard(CampaignData campaign, int invoiceIndex, int campaignIndex) {
    return Container(
        ...
        children: [
          ...
          Row(
            children: [
              ...
              Expanded(
                child: Text(
                  campaign.campaignName.isNotEmpty ? campaign.campaignName : 'Campaign ${campaignIndex + 1}',
                  style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 14),
                ),
              ),
              if (_invoices[invoiceIndex].campaigns.length > 1)
                ...
                Row(
                  children: [
                    ...
                    SizedBox(
                      width: 120,
                      child: TextField(
                        controller: TextEditingController(text: campaign.workingDays.isNotEmpty ? campaign.workingDays : 'Auto-calculated'),
                        readOnly: true,
                        decoration: InputDecoration(
            ...
            Column(
              ...
              children: [
                TextField(
                  ...
                  onChanged: (v) { campaign.dealershipName = v; widget.onInvoicesChanged(_invoices); },
                ...
                TextField(
                  ...
                  onChanged: (v) { campaign.dealershipAddress = v; widget.onInvoicesChanged(_invoices); },
                  maxLines: 2,
                  decoration: const InputDecoration(
                  ...
                  children: [
                    ...
                    ElevatedButton.icon(
                      onPressed: () {
                        ...
                        setState(() => campaign.gpsLocation = '28.6139° N, 77.2090° E');
                        widget.onInvoicesChanged(_invoices);
                      },
                ...
                if (campaign.photos.isEmpty)
                  InkWell(
                    ...
                    child: Container(
                      ...
                      child: Column(
                        children: [
                          Icon(Icons.add_a_photo, size: 32, color: AppColors.primary.withOpacity(0.6)),
                          const SizedBox(height: 8),
                  ...
                  Column(
                    ...
                    children: [
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                InkWell(
                                  ...
                                  child: const Icon(Icons.close, size: 14, color: AppColors.rejectedText),
                                ),
                              ],
                            ),
                          );
                      ...
                      const SizedBox(height: 12),
                      TextButton.icon(
                        onPressed: () => _pickPhotos(invoiceIndex, campaignIndex),
            ...
            Column(
              ...
              children: [
                ...
                if (campaign.costSummaryFile == null)
                  InkWell(
                    onTap: () => _pickCostSummaryFile(invoiceIndex, campaignIndex),
                    borderRadius: BorderRadius.circular(8),
                    child: Container(
                      width: double.infinity,
                  ...
                  Container(
                    ...
                    child: Row(
                      children: [
                        const Icon(Icons.check_circle, color: AppColors.approvedText, size: 20),
                        const SizedBox(width: 8),
                        Expanded(child: Text(campaign.costSummaryFile!.name, style: const TextStyle(fontWeight: FontWeight.w500), overflow: TextOverflow.ellipsis)),
                        IconButton(
                          onPressed: () { setState(() => campaign.costSummaryFile = null); widget.onInvoicesChanged(_invoices); },
                          icon: const Icon(Icons.close, size: 18), padding: EdgeInsets.zero, constraints: const BoxConstraints(),
                        ),
                      ],
                    ),
