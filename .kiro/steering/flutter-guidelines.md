# Flutter Guidelines & Best Practices
...
- Data and Presentation layers depend on Domain abstractions
...
- Implement concrete classes in data layer

#### Liskov Substitution Principle
- Subtypes must be substitutable for their base types
- Ensure implementations honor interface contracts
- Don't break expected behavior in subclasses
...
- Use `registerLazySingleton` for shared instances (repositories, services)
- Use `registerFactory` for BLoCs/Cubits (new instance per request)
- Use `registerSingleton` for truly global instances
- Initialize GetIt in `main()` before `runApp()`

#### Best Practices
- Register dependencies in order: Core → Data → Domain → Presentation
...
- **No business logic inside widgets** - all logic belongs in BLoC/use cases
...
#### Event Handling
- Define clear event classes for BLoC
...
- Use code generation with `riverpod_generator`
...
- Use `FutureBuilder` or `AsyncValue` (Riverpod) for async UI
- Cancel ongoing operations when widget is disposed
...
- Prefer `??` operator for default values
...
- Use `LayoutBuilder` for parent-constrained layouts
...
### Theme & Styling
- Define all colors, text styles, and dimensions in theme
...
- **Optimize critical render path** - minimize time to first paint
...
- Keep generated code in sync with source code
...
✅ **GetIt for Dependency Injection** - Use GetIt for ALL dependency injection
✅ **BLoC for ALL State Management** - No setState() for business logic
✅ **One Widget Per File** - MANDATORY, no exceptions
✅ **StatelessWidget Only** - Except for controllers (TextEditingController, etc.)
✅ **Max 300 Lines Per File** - Split files that exceed this limit
✅ **Multi-Layer Caching** - Implement Memory → Disk → Network caching
...
❌ **setState() for Business State** - Use BLoC/Cubit instead
