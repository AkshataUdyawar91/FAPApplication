...
{
    /// <inheritdoc />
    public partial class AddGstRateColumn : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            // Drop the unique index on GstCode first
            migrationBuilder.Sql("IF EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_StateGstMasters_GstCode') DROP INDEX IX_StateGstMasters_GstCode ON StateGstMasters;");

            // Drop GstCode column if it exists
            migrationBuilder.Sql("IF COL_LENGTH('StateGstMasters', 'GstCode') IS NOT NULL ALTER TABLE StateGstMasters DROP COLUMN GstCode;");

            // Add GstRate column if it doesn't exist
            migrationBuilder.Sql("IF COL_LENGTH('StateGstMasters', 'GstRate') IS NULL ALTER TABLE StateGstMasters ADD GstRate decimal(5,2) NOT NULL DEFAULT 18.00;");

            // Seed all 36 states if table is empty
...
END
ELSE
  UPDATE StateGstMasters SET GstRate = 18.00 WHERE GstRate = 0 OR GstRate IS NULL;
");
        }
