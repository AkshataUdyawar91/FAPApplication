...
public class TeamsBotService : TeamsActivityHandler
{
    ...
    /// Card button clicks (Activity.Value != null) are handled inline.
    /// All text messages are delegated to TeamsConversationRouter via a scoped service.
    /// Empty/null text returns silently per Req 1.4.
    /// </summary>
        ...
        CancellationToken cancellationToken)
    {
        ...
        var aadObjectId = turnContext.Activity.From?.AadObjectId ?? string.Empty;

        using var scope = _scopeFactory.CreateScope();
        var router = scope.ServiceProvider.GetRequiredService<TeamsConversationRouter>();
        await router.HandleAsync(text, aadObjectId, turnContext, cancellationToken);
    }
        ...
        CancellationToken cancellationToken)
    {
        foreach (var member in membersAdded)
        {
            ...
            // Send a brief welcome — the router handles full greeting on first text message
            await turnContext.SendActivityAsync(
                MessageFactory.Text("✅ ClaimsIQ Review Bot is ready. Type **hi** to get started."),
                cancellationToken);
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            // Store authenticated session so subsequent text messages are authorized via the router
            var convId = turnContext.Activity.Conversation?.Id ?? "";
            using var resolverScope = _scopeFactory.CreateScope();
            var approverResolver = resolverScope.ServiceProvider.GetRequiredService<ApproverResolver>();
            var resolvedApprover = await approverResolver.ResolveByCredentialsAsync(email, password, cancellationToken);
            if (resolvedApprover != null)
            {
                _authSessionStore.SetSession(convId, new BotAuthSession
                {
                    Step = AuthStep.Authenticated,
                    Email = email,
                    ResolvedUser = resolvedApprover,
            ...
            // Delegate to the conversational router to show pending submissions
            var aadObjId = turnContext.Activity.From?.AadObjectId ?? string.Empty;
            using var routerScope = _scopeFactory.CreateScope();
            var router = routerScope.ServiceProvider.GetRequiredService<TeamsConversationRouter>();
            await router.HandleAsync("pending", aadObjId, turnContext, cancellationToken);
        }
        catch (Exception ex)
        {
